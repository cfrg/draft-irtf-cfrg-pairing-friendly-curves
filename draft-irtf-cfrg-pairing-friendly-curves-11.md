...
area = "IRTF"
workgroup = "CFRG"
keyword = ["Internet-Draft","Request for Comments","Pairing-Friendly Curves","Eliptic Curve Cryptgraphy"]

[seriesInfo]
status = "informational"
name = "Internet-Draft"
value = "draft-irtf-cfrg-pairing-friendly-curves-11"
...
Currently, Boneh-Lynn-Shacham (BLS) signature schemes are being standardized [@I-D.boneh-bls-signature]
...
the Tate pairing, the optimal Ate pairing [@!Ver09], and so on. 
...
A BN curve [@!BN05] is a family of pairing-friendly curves proposed in 2005. 
...
The best-known algorithms for solving the discrete logarithm problems are based on Pollard's rho algorithm [@Pollard78] and Index Calculus [@HR83]. 
...
For instance, Barbulescu and Duquesne estimated that the security of the widely used BN256 curve was reduced to approximately 100 bits [@!BD18].
...
As shown in recent security evaluations for BLS12_381[@!BD18] [@!GMT19], its security level close to 128-bit but it is less than 128-bit. 
...
This serialization format is also adopted in [@I-D.boneh-bls-signature] [@zkcrypto].
...
To check these elements are so-called sub-group secure [@BCM15], implementors should validate if the elements have the correct order `r`. 
...
Recommended parameters are affected by the Cheon's attack which is a solving algorithm for the strong DH problem [@Cheon06]. 
...
When implementing the line function, implementers should consider the isomorphism of `E` and its twist curve `E'` so that one can reduce the computational cost of operations in `G_2` [＠CLN09] [＠KIK17].
...
This format applies to points on the BLS12_381 elliptic curves `E` and `E'`, whose parameters are given in (#BLS12_381). 
